...
local function pre_process(msg)
  ...
  if msg.action and msg.action.type then
    ...
    if action == 'chat_add_user_link' then
      ...
      print('<This is USER  ['..user_id..'] banned>')
...
--vardump(extra)
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '<User @'..member..' ['..member_id..']  banalled>')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '<User @'..member..' ['..member_id..'] unbanalled>')
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'This User ['..user_id..' ] banalled'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return '<This User ['..user_id..' ] removed from  banall  list>'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
